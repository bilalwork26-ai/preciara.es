...
async function main() {
  ...
  if (!result.ok) {
    ...
    console.error(result.sanitizedMessage);
    console.error("Revisa host, puerto, usuario, contraseña y que la base exista, pero nunca compartas la URL completa.");
    process.exitCode = 1;
    return;
  }

  console.log(`Conexión correcta a MySQL (respuesta en ${result.ms} ms).`);
  if (result.tablesReady) {
    const { categories, merchants, products, offers } = result.counts;
